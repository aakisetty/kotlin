...
fun generateNullCheckOnCallSite(descriptor: CallableDescriptor, v: InstructionAdapter) {
    val unsubstitutedReturnType = descriptor.original.returnType
    val inferredReturnType = descriptor.returnType

    if (unsubstitutedReturnType != null && inferredReturnType != null) {
        generateNullCheckOnCallSite(unsubstitutedReturnType, inferredReturnType, v)
    }
}

fun generateNullCheckOnCallSite(unsubstitutedReturnType: KotlinType, inferredReturnType: KotlinType, v: InstructionAdapter) {
    if (unsubstitutedReturnType.run { isTypeParameter() && isNullable() } && !inferredReturnType.isNullable()) {
